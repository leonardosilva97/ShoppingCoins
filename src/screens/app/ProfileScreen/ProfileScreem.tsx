...
export function Profile({navigation}: AppTabScreenProps<'Profile'>) {
  ...
  async function handleLogout() {
    await singOut();
    // Após fazer logout, navegamos para a rota de autenticação
    navigation.dispatch(
      CommonActions.reset({
        index: 0,
    ...
    <Screen scrolable style={$screen}>
      <Box paddingHorizontal="s24" alignItems="center" mb="s32">
        ...
        <Button
          width={132}
          preset="outline"
          presetText="paragraphSmall"
          title="Editar Perfil"
          onPress={() => {}}
        />
      </Box>
      <BoxBaseBody flex={1} paddingHorizontal="s24" pt="s32">
        <CardProfile icon="person" title="Detalhes do Perfil" />
        <CardProfile icon="bank" title="Detalhes da Conta" />
        <CardProfile icon="schedule" title="Histórico" />

        <Box justifyContent="center" alignItems="center" mt="s32">
          <Button width={100} title="Sair" onPress={handleLogout} />
        </Box>
      </BoxBaseBody>
    </Screen>
  );
